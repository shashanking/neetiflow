    ...
    try {
      ...
      for (var lead in leads) {
        try {
          ...
          print('Repository: Error processing lead ${lead.id}: $e');
          rethrow;
        }
      }
      print('Repository: ${rows.length} rows processed');

      rows.insert(0, headers);
      print('Repository: Converting to CSV format');

      final csv = const ListToCsvConverter().convert(rows);
      print('Repository: CSV conversion successful, size: ${csv.length} characters');
      
      final bytes = Uint8List.fromList(csv.codeUnits);
      print('Repository: Converted to bytes, size: ${bytes.length} bytes');
